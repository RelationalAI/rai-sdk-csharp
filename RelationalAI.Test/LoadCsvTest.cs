...
namespace RelationalAI.Test
{
    public class LoadCsvTests : UnitTest
    {
        ...
        public async Task LoadCsvtTest()
        {
            ...
            var loadRsp = await client.LoadCsvAsync(Dbname, engine.Name, "sample", Sample);
        ...
        public async Task LoadCsvNoHeaderTest()
        {
            ...
            var loadRsp = await client.LoadCsvAsync(Dbname, engine.Name, "sample_no_header", SampleNoHeader, opts);
            Assert.False(loadRsp.Aborted);
            Assert.Empty(loadRsp.Output);
            Assert.Empty(loadRsp.Problems);

            var rsp = await client.ExecuteV1Async(Dbname, engine.Name, "def output = sample_no_header");
        ...
        public async Task LoadCsvAltSyntaxTest()
        {
            ...
            var loadRsp = await client.LoadCsvAsync(Dbname, engine.Name, "sample_alt_syntax", SampleAltSyntax, opts);
            Assert.False(loadRsp.Aborted);
            Assert.Empty(loadRsp.Output);
            Assert.Empty(loadRsp.Problems);

            var rsp = await client.ExecuteV1Async(Dbname, engine.Name, "def output = sample_alt_syntax");
        ...
        public async Task LoadCsvWithSchemaTest()
        {
            ...
            var loadRsp = await client.LoadCsvAsync(Dbname, engine.Name, "sample", Sample, opts);
            Assert.False(loadRsp.Aborted);
            Assert.Empty(loadRsp.Output);
            Assert.Empty(loadRsp.Problems);
