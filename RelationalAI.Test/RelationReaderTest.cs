...
namespace RelationalAI.Test
{
    public class RelationReaderTest : UnitTest
    {
        ...
        {
            var reader = MockRelationReader();
            Assert.Equal(reader.ColumnCount(), 3);
            Assert.Equal(reader.TuplesCount(), 4);
        }
        ...
        void TupleTest()
        {
            ...
            {
                new List<object> {"output", 1, "foo", "w", "bar", "a", 1},
                new List<object> {"output", 1, "foo", "x", "bar", "b", 2},
                new List<object> {"output", 1, "foo", "y", "bar", "c", 3},
                new List<object> {"output", 1, "foo", "z", "bar", "d", 4},
            };

            var reader = MockRelationReader();

            for (int i = 0; i < reader.TuplesCount(); i++)
            {
                var tuple = reader.Tuple(i);
                Assert.Equal(JsonConvert.SerializeObject(tuple), JsonConvert.SerializeObject(expected[i]));
            }
        ...
        void PhysicalTest()
        {
            var expected = new List<List<object>>
            {
                new List<object> {"w", "a", 1},
                new List<object> {"x", "b", 2},
                new List<object> {"y", "c", 3},
                new List<object> {"z", "d", 4},
            };

            var reader = MockRelationReader().Physical();
            var tuples = reader.Tuples();
